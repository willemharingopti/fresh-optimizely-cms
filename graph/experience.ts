...
async function blogChildren(urlPrefix: string): Promise<ChildLink[]> {
  ...
  return (items ?? []).filter((c) =>
    (c?._metadata?.url?.default ?? "").startsWith(urlPrefix)
  );
}
...
async function childArticles(
  ...
  const articles = await Promise.all(
    (children ?? []).map((c) =>
      c?._metadata?.key ? loadArticleByKey(c._metadata.key) : null
    ),
  );
  return articles.filter((a): a is Article => a != null);
}
...
function hasArticleList(sections: Section[]): boolean {
  return sections.some((s) =>
    s.rows.some((r) =>
      r.columns.some((col) =>
        col.components.some((c) => c.__typename === "ArticleList")
      )
    )
  );
}
...
export async function loadPage(
  ...
  if (types.includes("_Experience")) {
    ...
    const articles = hasArticleList(sections)
      ? await childArticles(item._metadata?.key, signal)
      : [];
    return {
      kind: "experience",
      displayName: item._metadata?.displayName ?? "",
