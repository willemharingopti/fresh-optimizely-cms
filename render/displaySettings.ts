...
export function colorStyle(s: Settings): ColorStyle {
  switch (s.backgroundColor || s.sectionColor || "") {
    ...
    case "base_300":
      return {
        style:
          "background:var(--t-surface); border-block:1px solid var(--t-border);",
        dark: false,
      };
    case "info":
...
export function rowStyle(s: Settings): string {
  ...
  const width = s.rowWidth && s.rowWidth !== "inherit"
    ? ` ${px(s.rowWidth, SECTION_WIDTH, "")}`
    : "";
  return `display:flex; flex-wrap:wrap; gap:${gap}; justify-content:${justify}; ` +
    `align-items:${align};${
      my !== "0px" ? ` margin-block:${my};` : ""
    }${width}`;
}
...
export function columnStyle(s: Settings): string {
  ...
  const minw = s.minWidth && MINWIDTH[s.minWidth]
    ? ` min-width:${MINWIDTH[s.minWidth]};`
    : " min-width:220px;";
